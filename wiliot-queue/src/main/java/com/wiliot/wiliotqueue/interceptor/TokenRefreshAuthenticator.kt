...
import com.wiliot.wiliotqueue.utils.signedRequest
...
internal class TokenRefreshAuthenticator(
    private val mPrimaryTokenExpirationCallback: PrimaryTokenExpirationCallback?
): Authenticator {
    ...
    private fun Response.createSignedRequest(): Request? = try {
        val accessToken = WiliotQueue.configuration.authToken
        if (accessToken.isValidJwt("TokenRefreshAuthenticator[gw_access_token]").not())
            mPrimaryTokenExpirationCallback?.onPrimaryTokenExpired()
        request.signedRequest(WiliotQueue.configuration.authToken).also {
