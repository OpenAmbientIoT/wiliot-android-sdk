...
import com.wiliot.wiliotnetworkmeta.interceptor.MetaRequestSigningInterceptor
import com.wiliot.wiliotnetworkmeta.interceptor.MetaTokenRefreshAuthenticator
import com.wiliot.wiliotnetworkmeta.utils.coreApiBase
...
private object Common {
    ...
    fun provideTokenRefreshAuthenticator(): Authenticator {
        if (tokenRefreshAuthenticator == null) {
            tokenRefreshAuthenticator = MetaTokenRefreshAuthenticator(Wiliot.tokenExpirationCallback)
        }
    ...
    fun provideRequestSigningInterceptor(): Interceptor {
        if (requestSigningInterceptor == null) {
            requestSigningInterceptor = MetaRequestSigningInterceptor()
        }
    ...
    fun provideRequestHostSelectionInterceptor(): Interceptor {
        if (requestHostSelectionInterceptor == null) {
            requestHostSelectionInterceptor =
                MetaHostSelectionInterceptor(object : MetaHostSelectionInterceptor.MetaHostUrlSource {
